...
function crearProducto(req, res) {
  const { nombre, descripcion, precio, stock, categoria_id, activo, promocion, imagen_url } = req.body;
  ...
  pool.query(
    "INSERT INTO productos (nombre, descripcion, precio, stock, categoria_id, activo, promocion, imagen_url) VALUES ($1, $2, $3, $4, $5, $6, $7, $8) RETURNING *",
    [nombre, descripcion, precio, stock, categoria_id, activo ?? true, promocion ?? false, imagen_url],
    (error, resultado) => {
      if (error) {
...
function editarProducto(req, res) {
  ...
  pool.query(
    "UPDATE productos SET nombre = $1, descripcion = $2, precio = $3, stock = $4, categoria_id = $5, activo = $6, promocion = $7, imagen_url = $8 WHERE id = $9 RETURNING *",
    [nombre, descripcion, precio, stock, categoria_id, activo, promocion ?? false, imagen_url, id],
    (error, resultado) => {
      if (error) {
